...
Для компиляции и прошивки прошивки используйте команду:

```bash
make moonlander:srsatt:flash CC=arm-none-eabi-gcc CFLAGS="-Os -g -Wall -Wextra"
```

Где:
- `moonlander` - тип клавиатуры
- `srsatt` - имя раскладки 
- `flash` - автоматическая прошивка после компиляции
- `CC=arm-none-eabi-gcc` - компилятор для ARM процессора
- `CFLAGS="-Os -g -Wall -Wextra"` - флаги оптимизации и предупреждений
