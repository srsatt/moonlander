...
#define CASE_MAC(first, second) (system_current == SYSTEM_MACOS ? (second) : (first))
...
Key lang_calc_agnostic(Key key) {
  if (lang_current == 0) {
    ...
    switch (key) {
      ...
      case AG_SCLN: return CASE_MAC(RU_SCLN,RU_ASTR);
      case AG_COLN: return CASE_MAC(RU_COLN, RU_PERC);
      case AG_DQUO: return RU_DQUO;
      case AG_BSLS: return RU_BSLS;
#ifndef LANG_USE_ANSI
      case AG_PIPE: return RU_PIPE;
#endif
      case AG_DOT:  return CASE_MAC(RU_DOT, RU_QUES);
      case AG_COMM: return CASE_MAC(EN_COMM, RU_COLN);
#ifdef LANG_USE_ANSI
...
#endif
      case AG_QUES: return CASE_MAC(RU_QUES, RU_COMM);
      default:      return NONE_KEY;
    }
  }
}
